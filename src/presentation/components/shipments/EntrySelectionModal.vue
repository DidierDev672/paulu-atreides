      ...
      <div
        ...
        <div class="px-6 pt-4">
          <div class="relative">
            ...
            <input v-model="search" type="text" placeholder="Buscar por número, tipo o producto..."
        ...
        <div class="flex-1 overflow-y-auto px-6 py-4">
          ...
          <div v-else-if="entries.length === 0" class="flex flex-col items-center px-6 py-12 text-center">
            ...
            <p class="mt-2 max-w-sm text-sm leading-relaxed text-slate-500 dark:text-slate-400">
              Sin entradas registradas, el sistema no puede reflejar la existencia real de tus productos. Esto significa
              que <strong class="font-semibold text-slate-700 dark:text-slate-300">no podrás crear ordenes ni realizar
                despachos</strong> con productos que no tengan un ingreso documentado.
            </p>
            <p class="mt-3 max-w-sm text-xs leading-relaxed text-dune-status-warning dark:text-dune-status-warning">
              Cada producto sin entrada es un hueco en tu inventario — <strong class="font-semibold">registrar las
                entradas es el único camino para tener control real sobre tu stock</strong> y evitar pérdidas,
              sobrecostos y decisiones basadas en datos incorrectos.
            </p>
            <div class="mt-6">
              <button type="button"
              ...
              <template v-for="entry in filteredEntries" :key="entry.id">
                <tr
                  ...
                  <td class="px-2 py-3">
                    <input type="checkbox" :checked="selectedIds.has(entry.id)"
                      class="h-4 w-4 rounded border-slate-300 text-stellar-500 focus:ring-stellar-400" @click.stop
                      @change="toggle(entry.id)" />
                  </td>
                  <td class="px-2 py-3 font-mono text-xs font-medium text-slate-800 dark:text-slate-100">
                    {{ entry.entry_number }}</td>
                  <td class="px-2 py-3 text-slate-600 dark:text-slate-300">{{ entry.registered_date }}</td>
                  <td class="px-2 py-3 text-slate-600 dark:text-slate-300">
                    {{ movementLabels[entry.movement_type] || entry.movement_type }}</td>
                  <td class="px-2 py-3">
                    <div class="flex items-center gap-2">
                      <span
                        ...
                        {{ entry.details[0].product }} más {{ entry.details.length ? entry.details.length - 1 : 1 }}
                      </span>
                      <svg class="h-4 w-4 text-slate-400 transition"
                        :class="expandedId === entry.id ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24"
                ...
                <tr v-if="expandedId === entry.id" class="accordion-row">
                  <td colspan="5" class="bg-slate-50 px-6 py-4 dark:bg-slate-800/50">
                    <div
                      class="overflow-x-auto rounded-xl border border-slate-200 animate-fade-in dark:border-slate-700">
                      <table class="w-full text-left text-xs">
                        <thead>
                          <tr
                            class="border-b border-slate-200 bg-white text-xs font-semibold uppercase tracking-wider text-slate-400 dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <tbody>
                          <tr v-for="(detail, index) in entry.details" :key="detail.code"
                            class="border-b border-slate-100 bg-white last:border-0 dark:border-slate-800 dark:bg-slate-900 product-row"
                            :style="{ animationDelay: index * 0.08 + 's' }">
                            <td class="px-4 py-2">
                              <p class="font-medium text-slate-800 dark:text-slate-100">{{ detail.product }}</p>
                              <p class="text-slate-400">{{ detail.code }}</p>
                            </td>
                            <td class="px-4 py-2 text-slate-600 dark:text-slate-300">{{ detail.quantity }}
                              {{ detail.unit }}</td>
                            <td class="px-4 py-2 font-medium text-stellar-600 dark:text-stellar-400">
                              {{ formatCOP(detail.suggested_selling_price) }}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
        ...
        <div class="flex items-center justify-between border-t border-slate-200 px-6 py-4 dark:border-slate-700">
          <p class="text-sm text-slate-500 dark:text-slate-400">
            {{ selectedIds.size }} entrada{{ selectedIds.size !== 1 ? 's' : '' }}
